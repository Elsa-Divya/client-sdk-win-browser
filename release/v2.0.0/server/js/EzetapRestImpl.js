...
function EzetapRestImplTest(eze){
	...
	thi.takeChargePayment=function(charge,fn){
		try{
			//var charge=JSON.stringify(charge);

			var amount=charge.amount;
			
			

			ezecliWrapper.doPostCall(charge,function(response){
				...
				if("SUCCESS"==chargeResponse.status){
					...
					options.references.ref2=charge.chargeGroupId;
					options.references.ref3="CHARGE";
					
					if(charge.customer!=null){
						options.customer={};
						options.customer.mobileNo=charge.customer.mobileNo;
						options.customer.email=charge.customer.emailId;
					}
					
					thi.payCard(amount,"SALE",options,fn);
				}else{
					fn(500,responseObj("FAILURE",errorObj(chargeResponse.error.code,chargeResponse.error.message),null));
				}
